    ...
    <>
      ...
      >
        ...
        >
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4">
            {/* Mobile Phone Number - Show only on small screens */}
            <div className="lg:hidden flex items-center text-xs sm:text-sm text-gray-600">
              <FaPhoneAlt className="text-green-500 mr-1" size={12} />
              <span className="hidden xs:inline">+971 50 925 9667</span>
            </div>
            ...
            <Link href="/services">
              <button className="hidden sm:block bg-green-600 hover:bg-green-700 text-white font-semibold px-3 lg:px-4 py-2 rounded text-xs sm:text-sm lg:text-base transition-colors whitespace-nowrap">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <div
            className={`mobile-sidebar fixed left-0 top-0 h-full w-full max-w-sm bg-white shadow-2xl transform transition-all duration-300 ease-in-out overflow-y-auto ${
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gray-50">
              ...
              <button
                onClick={() => setMobileOpen(false)}
                className="flex items-center p-2 hover:bg-gray-200 rounded-md transition-colors"
              >
                <FaTimes size={16} />
              ...
              {navItems.map((item, index) => (
                <div key={index} className="border-b border-gray-100 last:border-b-0">
                  <div className="flex items-center justify-between">
                    <Link
                      href={item.href}
                      className="flex-1 block px-4 py-4 font-medium text-gray-700 hover:text-green-600 hover:bg-gray-50 transition-colors"
                      onClick={() => setMobileOpen(false)}
                    >
                      {item.title}
                    </Link>
