...
export default function PickupForm({ open, onClose }: PickupFormProps) {
  ...
  const [emailVerificationState, setEmailVerificationState] = useState<"unverified" | "pending" | "verified">(
    "unverified",
  )
    ...
    <div className="fixed inset-0 backdrop-blur-custom bg-black/30 flex items-center justify-center z-[99999] p-4 animate-in fade-in-0 duration-300">
      <Card className="w-full max-w-md rounded-lg shadow-xl border-0 bg-white overflow-hidden animate-in slide-in-from-bottom-4 duration-500">
        <CardHeader className="flex flex-row items-center justify-between p-4 pb-3">
          <CardTitle className="text-lg font-semibold text-gray-800">Schedule a Pickup</CardTitle>
              ...
              <div className="space-y-2">
                <div className="flex gap-2">
                  <Input
                    ...
                    required
                    className={`h-9 flex-1 ${emailVerificationState === "verified" ? "border-green-500" : ""}`}
                  />
                  <Button
                    type="button"
                    variant="outline"
                ...
                {otpMessage && (
                  <p className={`text-xs ${otpMessage.includes("✅") ? "text-green-600" : "text-red-600"}`}>
                    {otpMessage}
                  </p>
                )}
              </div>
